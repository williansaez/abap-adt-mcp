...
| `sessionExpired` | `code = SESSION_EXPIRED`, HTTP 401, or session-expired, login-page, identity-provider, `SAMLRequest`, SAML login/response/assertion or expired logon-ticket wording (an object name that merely contains `saml` does not count) | The SAP session expired or was never established. The server re-authenticates and retries once automatically; if this error still surfaces, call login for the destination and lock the object again before writing. | `login`, `lock` | The retry already happened; call `login`. On `basic` a persistent 401 is a wrong password. |
...
Non-fatal warnings cover `NODE_TLS_REJECT_UNAUTHORIZED=0` (removed at startup, not honoured), `insecureTls` destinations and reduced `Active toolsets`; a good stdio start prints `MCP ABAP ADT API server running on stdio` and the destination names. A `systems.json` entry with `user` and `password` but no `authType: "basic"` passes validation, defaults to `sso` and opens a browser on the first call; the legacy `SAP_URL` setup infers `basic` from `SAP_USER` and `SAP_PASSWORD` instead and prints `SAP_AUTH_TYPE not set; using basic`, while an explicit `SAP_AUTH_TYPE=sso` next to a password prints `the password is not used`.
