...
- Every error is JSON with `error` and `code`; when the server can classify it, `kind`, `httpStatus`, `hint` and `nextTools` are added. The kinds are `tlsCertificate`, `policyDenied`, `sessionExpired`, `csrf`, `locked`, `staleLockHandle`, `transportRequired`, `authorization`, `notFound`, `rateLimited`, `ambiguous400` and `serverError` (an unclassified error carries only the message). Follow the hint rather than retrying.
