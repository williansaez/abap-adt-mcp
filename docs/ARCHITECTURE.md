...
| `httpsAgent` | `https.Agent` from `buildHttpsAgent()` (`src/lib/tls.ts`), always present and destroyed with the destination. It carries `keepAlive: true` and an explicit `rejectUnauthorized`, `false` only under `insecureTls`, plus the CA, client certificate or PFX when the destination configures them. Stating the value rather than inheriting Node's default is what keeps an ambient `NODE_TLS_REJECT_UNAUTHORIZED=0` from reaching a destination that never asked for it |
...
2. **`src/lib/adtErrorHints.ts`**: `classifyAdtError()` maps an error (object or text) to an `AdtErrorKind`: `tlsCertificate`, `policyDenied`, `sessionExpired`, `csrf`, `locked`, `staleLockHandle`, `transportRequired`, `authorization`, `notFound`, `rateLimited`, `ambiguous400`, `serverError`, `unknown`. It reads the HTTP status from the error chain or the message text and matches SAP phrasings (`ExceptionResourceNoAccess`, "is being edited by", "not assigned to a transport", "SU53"). Every kind but `unknown` carries `hint` and `nextTools`; the `tlsCertificate` hint is built per failure (issuer, name, expiry) and, given the destination's `url` by the call handler, fills in the host and port of the `openssl` command it recommends.
...
4. **Policy gates**, only if the tool is a write a `systems.json` policy should be able to restrict, in `src/lib/policy.ts`:
   ...
   - `allowedTransports` checks the argument named in `TRANSPORT_ARGS: Record<string, string>`, another hardcoded per-tool-name table (`transport` for most source/object writes, `transportNumber` for `transportRelease`/`transportDelete`/`transportSetOwner`/`transportAddUser`). The three refactoring execute tools are the exception handled separately: their transport sits inside the `refactoring` proposal (`transport` and `affectedObjects[].transport`), parsed by `refactoringTransports()`, and a proposal without one is refused. Any other tool carrying a transport under a different argument name, or absent from this table altogether, is not checked by this gate even if `allowedTransports` is set, same silent-pass behaviour as an unlisted `allowedPackages` tool. The one exception is creation: `createTransport`, and `resolveTransport` with `createIfMissing: true`, are refused outright whenever `allowedTransports` is set, whatever the table says.
