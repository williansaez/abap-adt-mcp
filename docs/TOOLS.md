...
Generated by `npm run tools:docs` from the built server (v2.0.1). Do not edit by hand; edit the tool definitions and regenerate.
...
**What comes back.** `{status, result: GenericRefactoring as executed}`; `policyDenied` under `allowedPackages` (package of `adtObjectUri`). `allowedTransports` is evaluated on the transport inside the refactoring (and on every affected object's transport); a proposal without a transport is refused when that policy is set, so pick the transport at the preview step.
...
**Pitfalls.** The `git` toolset is gated on `/sap/bc/adt/abapgit/repos` (the abapGit ADT plugin must be installed) and is not in the `focused` preset; `deniedTools: ["toolset:git"]` keeps the tools listed but refuses them on that destination (`git*` misses `pushRepo`, `stageRepo`, `checkRepo`, `remoteRepoInfo` and `switchRepoBranch`).
