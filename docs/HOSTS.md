...
**Plugin manifest and skills.** `.claude-plugin/plugin.json` at the repository root declares the server under `mcpServers` as `npx -y abap-adt-mcp@<version>`, pinned to the release the manifest ships with (CI checks the pin against `package.json`, so a plugin host does not move to a newer npm release on restart), with only `SAP_SYSTEMS_FILE=${HOME}/.abap-adt-mcp/systems.json` in its `env` map (no `MCP_TOOLSETS`, more on that below); the two skills sit next to it in `skills/abap-adt-mcp/SKILL.md` (developing ABAP with these tools) and `skills/abap-adt-mcp-setup/SKILL.md` (installation and first health check). The repository is also its own marketplace (`.claude-plugin/marketplace.json`), so `/plugin marketplace add williansaez/abap-adt-mcp` followed by `/plugin install abap-adt-mcp@abap-adt-mcp` installs it as a Claude Code plugin: the manifest is what registers the server and the skills ship beside it, so at the time of writing no `claude mcp add` is needed; the host's own plugin documentation is the authority on what an install actually loads.

The manifest runs the published npm package at the pinned version, not a checkout's own build: loading the repository with Claude Code's local-plugin flag (`--plugin-dir <path>` at the time of writing) contributes the manifest, and the manifest names the npm package, so testing local source changes through the plugin route silently runs the pinned release instead of what you just built. To exercise a local build, point the checkout's `plugin.json` at `"command": "node", "args": ["<absolute path>/dist/index.js"]` first, or skip the plugin route and use `claude mcp add ... -- node /absolute/path/abap-adt-mcp/dist/index.js` as in the README's [Other ways to install](../README.md#other-ways-to-install) instead.

Because the manifest sets no `MCP_TOOLSETS`, a plugin install publishes all 173 tools by default, not the `focused` 114 recommended for Claude Code elsewhere on this page ([Choosing MCP_TOOLSETS](#choosing-mcp_toolsets-per-host-and-task)), and the manifest has no per-install override for it. Getting `focused` from the plugin route means loading an edited checkout's `plugin.json` with `--plugin-dir`, or registering the server a second way (`claude mcp add` or a project `.mcp.json`) alongside or instead of the plugin.
