...
- `dotenv` is loaded with `quiet: true`. Version 17 prints an "injected env" line to stdout by default, and in stdio mode stdout is the JSON-RPC channel; a `.env` next to `package.json` would have broken every host. The option is a no-op on 16.
